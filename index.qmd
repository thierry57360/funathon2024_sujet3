...
Avec ce sujet, nous vous proposons de reconstruire une interface utilisateur qui se rapproche de celle du [site FlightRadar24](https://www.flightradar24.com/) avec Python.
...
- Développement d'un site interactif avec [Dash](https://dash.plotly.com/), un framework Python réputé servant à construire des applications de visualisation de données. 

# Déroulement du sujet

Ce sujet comporte 4 parties :

1. Dans la première partie, nous explorerons les données disponibles via une [interface de programmation](https://github.com/JeanExtreme002/FlightRadarAPI);
2. Dans la deuxième partie, nous écrirons une première version de l'application de visualisation;
3. Dans la troisième partie, nous raffinerons l'application;
4. Dans une quatrième partie optionnelle, nous verrons comment déployer l'application sur le [SSP Cloud](https://datalab.sspcloud.fr/) et la rendre accessible depuis Internet.

# Initialisation du projet

## Récupérer le projet avec `Git` {{< fa brands git-alt >}}

Pour se lancer dans le sujet, commencer par ouvrir un service [Vscode-python](https://datalab.sspcloud.fr/launcher/ide/vscode-python) (par exemple), en modifiant l'onglet *Networking* de la configuration (cocher la case "Enable a custom service port"). Dans le service, cloner le projet depuis un Terminal avec la commande
```
